...
export class UserServiceTTT {
  ...
  login(user) {
    ...
    return this.http.post(this._loginApi,
      body, <RequestOptionsArgs>{ headers: headers, withCredentials: true })
  ...
  getUsers() {
    return this.http.get(this._apiBase + '/api/users?limit=5&desc=true',
      <RequestOptionsArgs>{ withCredentials: true })
      .map((res: Response) => res.json())
      .catch(this.handleError);
  }
  ...
  getMe() {
    return this.http.get(this._apiBase + '/api/users/me/',
      <RequestOptionsArgs>{ withCredentials: true })
